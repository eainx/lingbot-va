...
class VA_Server:
    ...
    def _reset(self, prompt=None):
        ...
        if hasattr(self, 'server_video_frames'):
            self.server_video_frames = []
        if hasattr(self, 'server_attn_frames'):
    ...
    def _infer(self, obs, frame_st_id=0):
        ...
        attn_map = target_layer.last_attn_map # [B, H, Q, K]
    ...
    def infer(self, obs):
        ...
        if reset:
            ...
            if not hasattr(self, 'server_video_frames'):
                self.server_video_frames = []
            self.server_video_frames.extend(decoded_prediction) # 생성된 원본 프레임 추가
            export_to_video(self.server_video_frames, 
                            os.path.join(self.exp_save_root, "server_generated_video.mp4"), fps=10)
            
            if attn_map is not None:
                vis_chunk = overlay_attention_to_video(v_grid_id, attn_map[0], decoded_prediction)
    ...
    def generate(self):
        ...
        for chunk_id in range(self.job_config.num_chunks_to_infer):
            actions, latents, attn_map, v_grid_id = self._infer(init_obs, frame_st_id=(chunk_id * self.job_config.frame_chunk_size))
            
            decoded_chunk = self.decode_one_video(latents, output_type='np')[0]
            
            if attn_map is not None:
                vis_chunk = overlay_attention_to_video(v_grid_id, attn_map[0], decoded_chunk)
                attn_vis_frames.extend(vis_chunk)
        ...
        if attn_vis_frames:
            export_to_video(attn_vis_frames, os.path.join(self.save_root, "attn_debug.mp4"), fps=10)
            logger.info(f"✅ Attention debug video saved to {self.save_root}/attn_debug.mp4")
