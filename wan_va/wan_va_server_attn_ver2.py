...
class VA_Server:
    ...
    def __init__(self, job_config):
        ...
        self.attention_visualization = job_config.get('attention_visualization', False)


    def overlay_attention_to_video(self, grid_id, attn_map, original_video, head_idx=0, alpha=0.6):
        """
        grid_id: [3, L] (f, h, w)
        attn_map: [Heads, Q, K] 
        original_video: [F, H, W, 3] (0-255 numpy)
        """
        num_video_tokens = grid_id.shape[-1]
        
        # attn_map[head, query_idx, key_idx]
        weights = attn_map[head_idx, -1, :num_video_tokens].cpu().numpy()

        f_ids = grid_id[0, :].cpu().numpy().astype(int)
        h_ids = grid_id[1, :].cpu().numpy().astype(int)
        w_ids = grid_id[2, :].cpu().numpy().astype(int)

        max_f, max_h, max_w = f_ids.max() + 1, h_ids.max() + 1, w_ids.max() + 1
        heatmap_3d = np.zeros((max_f, max_h, max_w))
        
        for i in range(num_video_tokens):
            heatmap_3d[f_ids[i], h_ids[i], w_ids[i]] = weights[i]

        overlaid_frames = []
        for t in range(len(original_video)):
            frame = original_video[t]
            ...
            h_map = heatmap_3d[min(t, max_f-1)]
            h_map = (h_map - h_map.min()) / (h_map.max() - h_map.min() + 1e-8)
            
            h_map_resized = cv2.resize(h_map, (w, h), interpolation=cv2.INTER_LINEAR)
            heatmap_color = cv2.applyColorMap(np.uint8(255 * h_map_resized), cv2.COLORMAP_JET)
            heatmap_color = cv2.cvtColor(heatmap_color, cv2.COLOR_BGR2RGB)
            
            overlaid = cv2.addWeighted(frame, 1 - alpha, heatmap_color, alpha, 0)
    ...
    def _infer(self, obs, frame_st_id=0):
        ...
        with (
            ...
            if self.attention_visualization:
                raw_model = self.transformer.module if hasattr(self.transformer, 'module') else self.transformer
                target_layer = raw_model.blocks[-1].attn1

            for i, t in enumerate(tqdm(action_timesteps)):
                last_step = i == len(action_timesteps) - 1

                if last_step and self.attention_visualization:
                    target_layer.save_attention = True
        ...
        if self.attention_visualization:
            attn_map = target_layer.last_attn_map
            target_layer.save_attention = False
            video_grid_id = get_mesh_id(latents.shape[-3] // self.job_config.patch_size[0],
                                    latents.shape[-2] // self.job_config.patch_size[1],
                                    latents.shape[-1] // self.job_config.patch_size[2], 
        ...
        torch.cuda.empty_cache()
        return actions, latents, attn_map, video_grid_id
    ...
    def infer(self, obs):
        ...
        if reset:
            ...
            if not self.attention_visualization:
                action, _ = self._infer(obs, frame_st_id=self.frame_st_id)
            else:
                action, latents, attn_map, v_grid_id = self._infer(obs, frame_st_id=self.frame_st_id)
                decoded_prediction = self.decode_one_video(latents, output_type='np')[0]

                if not hasattr(self, 'server_video_frames'):
                    self.server_video_frames = []
                self.server_video_frames.extend(decoded_prediction)
                export_to_video(self.server_video_frames, 
                                os.path.join(self.exp_save_root, "server_generated_video.mp4"), fps=10)
                
                if attn_map is not None:
                    vis_chunk = self.overlay_attention_to_video(v_grid_id, attn_map[0], decoded_prediction)
                    
                    if not hasattr(self, 'server_attn_frames'):
                        self.server_attn_frames = []
                    self.server_attn_frames.extend(vis_chunk)
                    
                    export_to_video(self.server_attn_frames, 
                                    os.path.join(self.exp_save_root, "server_generated_attn.mp4"), fps=10)

                    self.frame_st_id += latents.shape[2]

                    return dict(action=action, attention=attn_map.cpu().numpy())
                else:
